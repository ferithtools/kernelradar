...
- You need a **mature, multi-year track record**: kernelradar is
  v0.1.2. Pilot it before paging anyone.
...
curl -fsSLO https://github.com/ferithtools/kernelradar/releases/download/v0.1.2/kernelradar-0.1.2-linux-x86_64.tar.gz
curl -fsSLO https://github.com/ferithtools/kernelradar/releases/download/v0.1.2/kernelradar-0.1.2-linux-x86_64.tar.gz.sha256
...
sha256sum -c kernelradar-0.1.2-linux-x86_64.tar.gz.sha256 \
    || { echo "TAMPERED - do not install"; exit 1; }

PIN=$(curl -fsSL https://raw.githubusercontent.com/ferithtools/kernelradar/v0.1.2/release-checksums/v0.1.2/kernelradar-0.1.2-linux-x86_64.tar.gz.sha256 | awk '{print $1}')
PUB=$(awk '{print $1}' kernelradar-0.1.2-linux-x86_64.tar.gz.sha256)
[ "$PIN" = "$PUB" ] || { echo "release-attached SHA does not match in-repo pin"; exit 1; }

# 3. Extract + run the bundled installer.
tar -xzf kernelradar-0.1.2-linux-x86_64.tar.gz
cd kernelradar-0.1.2-linux-x86_64
